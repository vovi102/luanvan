...
**Architecture:** Pure contracts and transformations generate and enumerate anchor-safe candidates without I/O. A deterministic allocator owns exact quotas and artifact validation; a thin Click CLI owns JSONL loading, locked/journaled publication, recovery, and manifest hashes. Full-size generated fixtures keep tests independent of the credential-gated Stage C artifact.
...
- Each final file is atomically replaced after validation; an advisory lock, unique staging, backups, and a durable journal serialize the pair and recover interrupted publication.
...
- `NoiseConfig()` accepts only seed 42 and the exact 38/38/37/37 quotas.
- `transform_question` returns `str | None`; `None` means that record is ineligible for that noise type.

- [x] **Step 1: Write strict contract and dictionary tests**
...
- [x] **Step 2: Write pure transformation and anchor-protection tests**
...
- [x] **Step 4: Implement contracts and the versioned dictionary**
...
- [x] **Step 5: Implement protected spans and four single-edit transforms**
...
- [x] **Step 1: Write a full-size valid Stage C fixture factory**
...
- [x] **Step 2: Write RED tests for exact deterministic allocation**
...
- [x] **Step 3: Write RED tests for immutability and rejection gates**
...
- [x] **Step 5: Implement deterministic candidate allocation**
...
- [x] **Step 6: Implement complete Stage D validation and stats**

`NoiseStats` contains source/output/noisy counts, per-type counts, unique raw count, unique normalized count, and mean/max nonzero distance. Recompute every source `nl_normalized`. Compare each noisy row to the source after removing only `id`, `nl`, `nl_normalized`, `noise_parent_id`, `nl_original`, `noise_type`, `noise_seed`, and `noise_distance`; require noisy text to match one enumerated transform for its declared type and dictionary. Re-run anchor validation; require finite distance metadata equal to fresh distance.

- [x] **Step 7: Run Task 1+2 GREEN and static checks**
...
### Task 3: Recoverable artifacts, manifest, CLI, and notebook
...
- Reuses T3.3 `deterministic_audit_ids`; canonical JSONL bytes, lock, journal, backups, recovery, and coordinated publication live in the Stage D artifact module.

- [x] **Step 1: Write RED tests for manifest evidence**
...
- [x] **Step 2: Write RED CLI tests for missing input and recoverable publication**
...
- [x] **Step 4: Implement manifest construction and validation**
...
- [x] **Step 5: Implement Click orchestration**

Default paths are `data/dataset/raw/synthetic-stage-c.jsonl`, `synthetic-stage-d.jsonl`, and `noise-config.json`. Generate mode loads source and dictionary, injects/validates entirely in memory, then publishes canonical Stage D bytes and manifest under one process lock using unique staging, durable backups, and a journal. Caught errors roll back immediately; `validate-output` acquires the same lock and recovers an interrupted run before reading.

- [x] **Step 6: Create the analysis notebook**
...
- [x] **Step 7: Run focused GREEN and static checks**
...
- [x] **Step 1: Run the default CLI and capture the honest gate**
...
- [x] **Step 2: Rewrite the task from SPARQL/probabilistic wording to the approved contract**

Mark implementation/test/recoverable-publication acceptance checked. Leave final 3,150 artifact, 30-row manual audit, and source/output hash evidence unchecked. State `implementation complete; artifact blocked by T3.3 credential gate` without reducing acceptance.

- [x] **Step 3: Add the decision-log entry**
...
- [x] **Step 4: Run final verification from the clean implementation state**
