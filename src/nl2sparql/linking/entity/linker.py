...
from nl2sparql.linking.entity.stopwords import ENTITY_LINKER_FUNCTION_WORDS

_ADDRESS_RE = re.compile(r"(?<!\w)0x[0-9a-f]{40}(?!\w)", re.IGNORECASE)
_TOKEN_RE = re.compile(r"[^\W_]+", re.UNICODE)
# Exact aliases run before this finite function-word policy, so a legitimate
# entity alias also present in Snowball remains available at the exact stage.
_STOPWORDS = ENTITY_LINKER_FUNCTION_WORDS
